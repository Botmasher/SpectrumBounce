...
public class LightTestMusicManager : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = 0; i < numSamples; i++) {
			spectrumBlocks.Add (Instantiate (spectrumBlock, new Vector3 (spectrumXOrigin+i*0.2f, -2f, 0f), Quaternion.identity) as GameObject);
		}
	...
	void MoveSpectrumWithMusic () {
		...
		for (int i = 1; i < currentSpectrumData.Length-1; i++) {
			...
			if (GetComponent<AudioSource>().isPlaying) {
				...
				if (songVolume > 0f) {
					spectrumBlocks[i].transform.localScale = Vector3.Lerp (spectrumBlocks[i].transform.localScale, new Vector3 ( spectrumBlocks[i].transform.localScale.x, 0.2f + 5f*(currentSpectrumData[i]*(1+songVolume))*i*Mathf.Log(currentSpectrumData[i]*currentSpectrumData[i]*i*i), 0.1f ), 9f * Time.deltaTime);
				} else if (songVolume < 0f) {
					spectrumBlocks[i].transform.localScale = Vector3.Lerp (spectrumBlocks[i].transform.localScale, new Vector3 ( spectrumBlocks[i].transform.localScale.x, 0.2f + 5f*(currentSpectrumData[i]/(1+Mathf.Abs(songVolume)))*i*Mathf.Log(currentSpectrumData[i]*currentSpectrumData[i]*i*i), 0.1f ), 9f * Time.deltaTime);
				} else {
					spectrumBlocks[i].transform.localScale = Vector3.Lerp (spectrumBlocks[i].transform.localScale, new Vector3 ( spectrumBlocks[i].transform.localScale.x, 0.2f + 5f*currentSpectrumData[i]*i*Mathf.Log(currentSpectrumData[i]*currentSpectrumData[i]*i*i), 0.1f ), 9f * Time.deltaTime);
				}

//				// change y-scale according to output data, accounting for volume changes
//				if (songVolume > 0f) {
//					spectrumBlocks[i].transform.localScale = Vector3.Lerp (spectrumBlocks[i].transform.localScale, new Vector3 ( spectrumBlocks[i].transform.localScale.x, 0.2f + 7f*(currentSpectrumData[i]*(1+songVolume)), 0.1f ), 4f * Time.deltaTime);
//				} else if (songVolume < 0f) {
//					spectrumBlocks[i].transform.localScale = Vector3.Lerp (spectrumBlocks[i].transform.localScale, new Vector3 ( spectrumBlocks[i].transform.localScale.x, 0.2f + 7f*(currentSpectrumData[i]/(1+Mathf.Abs(songVolume))), 0.1f ), 4f * Time.deltaTime);
//				} else {
//					spectrumBlocks[i].transform.localScale = Vector3.Lerp (spectrumBlocks[i].transform.localScale, new Vector3 ( spectrumBlocks[i].transform.localScale.x, 0.2f + 7f*currentSpectrumData[i], 0.1f ), 4f * Time.deltaTime);
//				}
